...
public class ContactPickerAdapter extends RecyclerView.Adapter<ContactPickerAdapter.ViewHolder> {
    ...
    public enum FilterType { ALL, CONTACTS, GROUPS }
    ...
    private void applyFilter() {
        ...
        List<ContactData> typeFiltered = new ArrayList<>();
        for (ContactData contact : allContacts) {
            boolean isGroup = contact.isGroup();
            if (currentFilter == FilterType.ALL ||
                (currentFilter == FilterType.CONTACTS && !isGroup) ||
                (currentFilter == FilterType.GROUPS && isGroup)) {
                typeFiltered.add(contact);
            }
        }

        // Then: apply search filter
        if (currentQuery.isEmpty()) {
            filteredContacts.addAll(typeFiltered);
        } else {
            ...
            for (ContactData contact : typeFiltered) {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(ContactData contact, boolean isSelected, OnContactSelectedListener listener) {
            ...
            if (contact.getPhotoUri() != null) {
                ...
                // Use group icon for groups, person icon for contacts
                int iconRes = contact.isGroup() ? R.drawable.ic_contacts : R.drawable.ic_general;
                imageContactPhoto.setImageResource(iconRes);
